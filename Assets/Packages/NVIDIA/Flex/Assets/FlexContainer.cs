...
// AANGEPAST OP BASIS VAN: https://github.com/neuroailab/flex-ml-agents/
...
namespace NVIDIA.Flex
{
    ...
    public class FlexContainer : ScriptableObject
    {
        ...
        public int substepCount { get { return m_substepCount; } set { m_substepCount = Mathf.Max(value, 1); } } // deze zit er niet in eigenlijk

        public int iterationCount { get { return m_iterationCount; } set { m_iterationCount = Mathf.Max(value, 1); } } // deze zit er niet in eigenlijk
        ...
        public float staticFriction { get { return m_staticFriction; } set { m_staticFriction = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float dynamicFriction { get { return m_dynamicFriction; } set { m_dynamicFriction = Mathf.Clamp(value, 0.0f, m_staticFriction); } } // deze zit er niet in eigenlijk

        public float particleFriction { get { return m_particleFriction; } set { m_particleFriction = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float restitution { get { return m_restitution; } set { m_restitution = Mathf.Clamp01(value); } } // deze zit er niet in eigenlijk

        public float adhesion { get { return m_adhesion; } set { m_adhesion = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float sleepThreshold { get { return m_sleepThreshold; } set { m_sleepThreshold = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float maxSpeed { get { return m_adhesion; } set { m_maxSpeed = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float maxAcceleration { get { return m_maxAcceleration; } set { m_maxAcceleration = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float shockPropagation { get { return m_shockPropagation; } set { m_shockPropagation = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float dissipation { get { return m_dissipation; } set { m_dissipation = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float damping { get { return m_damping; } set { m_damping = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float drag { get { return m_drag; } set { m_drag = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float lift { get { return m_lift; } set { m_lift = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float cohesion { get { return m_cohesion; } set { m_cohesion = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float surfaceTension { get { return m_surfaceTension; } set { m_surfaceTension = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float viscosity { get { return m_viscosity; } set { m_viscosity = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float vorticityConfinement { get { return m_vorticityConfinement; } set { m_vorticityConfinement = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float anisotropyScale { get { return m_anisotropyScale; } set { m_anisotropyScale = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float anisotropyMin { get { return m_anisotropyMin; } set { m_anisotropyMin = Mathf.Clamp01(value); } } // deze zit er niet in eigenlijk

        public float anisotropyMax { get { return m_anisotropyMax; } set { m_anisotropyMax = Mathf.Clamp01(value); } } // deze zit er niet in eigenlijk

        public float smoothing { get { return m_smoothing; } set { m_smoothing = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float solidPressure { get { return m_solidPressure; } set { m_solidPressure = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float freeSurfaceDrag { get { return m_freeSurfaceDrag; } set { m_freeSurfaceDrag = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float buoyancy { get { return m_buoyancy; } set { m_buoyancy = Mathf.Clamp(value, -10.0f, 10.0f); } } // deze zit er niet in eigenlijk

        public float particleCollisionMargin { get { return m_particleCollisionMargin; } set { m_particleCollisionMargin = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float shapeCollisionMargin { get { return m_shapeCollisionMargin; } set { m_shapeCollisionMargin = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

        public float relaxationFactor { get { return m_relaxationFactor; } set { m_relaxationFactor = Mathf.Max(value, 0.0f); } } // deze zit er niet in eigenlijk

                public float collisionDistance { get { return m_collisionDistance; } set { m_collisionDistance = value; } } // dit
        ...
        public int fluidIndexCount { get { return m_fluidIndexCount; } }
                ...
                public int maxParticles { get { return m_maxParticles; } set { m_maxParticles = value; } } // dit

                public int numParticles { get { return m_numParticles; } } // dit

                public List<FlexActor> actors { get { return m_actors; } } // dit: List of all the actors
        ...
        public void AddActor(FlexActor actor)
        {
            if (m_actorCount == 0) CreateContainer();
                    actor.id = m_actorCount; // dit: geef elke actor een ID op basis van de "hoeveelste" het is
            ++m_actorCount;
                    m_actors.Add(actor); // dit: Voeg de actor toe aan de actor lijst
                    m_numParticles += actor.asset.particles.Length; // dit: Voeg het aantal particles toe als er een nieuwe actor komt
        }

        public void RemoveActor(FlexActor actor)
        {
                    m_numParticles -= actor.asset.particles.Length; // dit: Hier wordt het aantal particles verminderd bij verwijdering van actor
                    m_actors.Remove(actor); // dit: Verwijder de actor uit de list
            --m_actorCount;
        ...
        void UpdateDrawFluid(ParticleData _particleData) // disabled this because the fluid rendering is broken in URP anyway, and this function would sometimes cause crashes and causes unneeded overhead
        {
        ...
        int m_actorCount = 0;
                ...
                List<FlexActor> m_actors = new List<FlexActor>(); // dit: Lijst met actors waar makkelijk kan worden toegevoegd/verwijderd
        ...
        Material m_fluidMaterial;
                ...
                private int m_numParticles = 0; // dit: het totaal aantal particles in de container
