...
namespace NVIDIA.Flex  // drawfluid not working so removed the functuionality to prevent unneeded overhead
{
    [ExecuteInEditMode]
    ...
    public class _auxFlexDrawFluid : MonoBehaviour
    {
        ...
        void OnEnable()
        {
            //Create(); // drawfluid not working so removed the functuionality to prevent unneeded overhead
        }
        ...
        {
            Debug.Log(_particleData.container.name);
