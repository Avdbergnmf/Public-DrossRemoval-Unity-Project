...
namespace NVIDIA.Flex
{
    ...
    public class FlexActor : MonoBehaviour
    {
        ...
        public FlexContainer container
        {
            ...
            public Vector4[] particles // dit: een public function die de m_particles array teruggeeft wanneer het wordt aangeroepen (evt van buiten het script/de class)
            {
                get { return m_particles; }
            }

            public Vector3[] velocities // dit: een public function die de m_velocities array teruggeeft wanneer het wordt aangeroepen (evt van buiten het script/de class)
            {
        ...
        void AcquireAsset()
        {
            if (subclassAsset)
            {
                ...
                m_currentAsset.onAfterRebuild += OnAfterRecreate;
                        m_particles = new Vector4[m_currentAsset.particles.Length]; // dit: maak vector aan voor positie data van de particles van deze asset
                        m_velocities = new Vector3[m_currentAsset.particles.Length]; // dit: maak een vector aan voor de velocity data van deze asset
            }
        }
        ...
        void ReleaseAsset()
        {
            if (m_currentAsset)
            {
                ...
                m_currentAsset.onAfterRebuild -= OnAfterRecreate;
                m_currentAsset = null;
                        m_particles = null; // dit: Verwijder de positie vector
                        m_velocities = null; // dit: verwijder de velocity vector
            }
        }
        ...
        protected virtual void CreateInstance()
        {
            if (m_currentContainer && m_currentContainer.handle && m_currentAsset && m_currentAsset.handle)
            {
                ...
                if (m_fluid) flags |= Flex.Phase.Fluid; // if its a fluid (for example)
                m_phase = Flex.MakePhase(group, flags); // add the fluid flag
        ...
        protected virtual void OnFlexUpdate(FlexContainer.ParticleData _particleData)
        {
            ...
            UpdateBounds(_particleData);
                    ...
                    UpdateParticles(_particleData); // dit: Nieuwe functie om de particle data te updaten
        ...
        void CreateActor()
        {
                    AcquireAsset(); // dit: switched order with AddToContainer()
            AddToContainer();
            CreateInstance();
        }
        ...
        void DestroyActor()
        {
            DestroyInstance();
            RemoveFromContainer();
                    ReleaseAsset(); // dit: switched order with RemoveFromContainer()
        }
        ...
        void UpdateBounds(FlexContainer.ParticleData _particleData)
        {
            ...
            m_bounds.SetMinMax(boundsMin, boundsMax);
                ...
                void UpdateParticles(FlexContainer.ParticleData _particleData) // dit: Nieuwe functie om particle data te updaten
                {
                    if (m_container != null && m_indices != null && m_indices.Length > 0)
                    {
                        _particleData.GetParticles(m_indices[0], m_indices.Length, m_particles); // dit: dit was de functie die ik nodig had, geen idee waarom zo moeilijk was om te vinden
                        _particleData.GetVelocities(m_indices[0], m_indices.Length, m_velocities); // dit: dit was de functie die ik nodig had, geen idee waarom ik heb nooit gevonden heb...
                    }
                }
        ...
        FlexContainer m_container;
            [SerializeField]
            Vector4[] m_particles; // dit: alle particle posities om weer te geven in de debugger
            [SerializeField]
            Vector3[] m_velocities;  // dit: alle particle velocites om weer te geven in de debugger
        [SerializeField]
