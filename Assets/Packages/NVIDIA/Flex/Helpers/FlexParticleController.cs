    ...
    public class FlexParticleController : MonoBehaviour
    {
        ...
        void OnFlexUpdate(FlexContainer.ParticleData _particleData)
        {
            ...
            else if (actor)
            {
                ...
                //m_particleSystem.Clear();
                //m_particleSystem.Emit(indices.Length);
                if (m_particles.Length != indexCount) m_particles = new ParticleSystem.Particle[indexCount]; // if it changed size
                Color32 startColor = main.startColor.Evaluate(0); // <<<<<<<I can fuck with the color and size here!
                float startSize = main.startSize.Evaluate(0);

                ParticleSystem.Particle[] p = m_particles;

                for (int i = 0; i < indexCount; ++i)
                {
                    p[i].velocity = _particleData.GetVelocity(indices[i]);
                    p[i].position = (Vector3)_particleData.GetParticle(indices[i]) + p[i].velocity * (time - Time.fixedDeltaTime);
                    p[i].remainingLifetime = m_particleSystem.main.startLifetime.Evaluate(0);
                    p[i].startLifetime = p[i].remainingLifetime;
                    p[i].startColor = startColor;
                    p[i].startSize = startSize;
