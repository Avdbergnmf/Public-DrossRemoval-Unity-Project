...
public class HapticPlugin : MonoBehaviour
{
    ...
    //Alex dingetjes
    public int buttonID = 1;       //--> index of the button assigned to grabbing.  Defaults to the first button
    public bool ButtonActsAsToggle = false;	//--> Toggle button? as opposed to a press-and-hold setup?  Defaults to off.
    public GameObject controlledObject = null; // Alex: added in this. This is the object that will be controlled

    public bool enableCustomControlPoint = false; // if this is disabled, the controlledobject is grabbed at its origin.
    [ConditionalField(nameof(enableCustomControlPoint))] public Transform customControlPoint = null; // The location of the point you want to control, this always takes the global position of the transform object you input
    private Vector3 relativeControlPoint;
    private Vector3 controlPoint;

    [Header("ReadOnly Attributes")]

    [ReadOnly] public GameObject touching = null;  //!< (Readonly) Reference to the touchable unity gameobject currently being touched
    [ReadOnly] public float touchingDepth = 0; //!< (Readonly) Depth (ie: pressure) the 'touching' object is being touched.

    [ReadOnly] public int hHD = -1;
    [ReadOnly] public string device_SerialNumber = "-Not Connected-";   //!< (Readonly) Serial number of the haptic device.
    [ReadOnly] public string device_Model = "-Not Connected-"; //!< (Readonly) Model of the haptic device

    [ReadOnly] public Vector3 stylusPositionRaw;    //!< (Readonly) Stylus position, in device coordinates.
    [ReadOnly] public Vector3 stylusVelocityRaw;    //!< (Readonly) Stylus velocity, in device coordinates.
    [ReadOnly] public Matrix4x4 stylusTransformRaw; //!< (Readonly) Stylus transform, in device coordinates.
    [ReadOnly] public int[] Buttons;                //!< (Readonly) Array of buttons. 1 is pressed, 0 is unpressed.
    [ReadOnly] public int inkwell;                  //!<! (Readonly) 1 if inkwell button is triggered. 0 otherwise. (Not all devices have inkwells.)


    [ReadOnly] public Vector3 proxyPositionRaw;         //!< (Readonly) Proxy position, in device coordinates.
    [ReadOnly] public Quaternion proxyOrientationRaw;   //!< (Readonly) Proxy Orientation, in device coordinates.
    [ReadOnly] public Matrix4x4 proxyTransformRaw;      //!< (Readonly) Proxy Transform in device coordinates.
    [ReadOnly] public Vector3 proxyNormalRaw;
    ...
    Matrix4x4 stylusTransformRef;
    Vector3 omniPosCorr;
    Vector3 manipPosCorr;
    public GameObject grabbing = null;         //!< Reference to the object currently grabbed
    ConfigurableJoint joint = null; // made this a configurablejoint
    public bool enableGrabbing = false;
    private FixedJoint jointPause = null;
    private bool buttonFilter = false;

    // Logging Attributes
    public springProperties OmniSpring;
    public struct springProperties
    {
        public string configName;
        public double stiffness;
        public double damping;
        public double[] anchorPosition3;
        public double[] anchorVelocity3;
    ...
    void Start()
    {
        ...
        if (enableCustomControlPoint) //if a custom control point is set
        {
            ...
            relativeControlPoint = controlledObject.transform.localToWorldMatrix.inverse.MultiplyPoint3x4(customControlPoint.position);
    ...
    void FixedUpdate()
    {
        ...
        controlPoint = controlledObject.transform.localToWorldMatrix.MultiplyPoint3x4(relativeControlPoint);
        ...
        if (enableGrabbing)
        {
            ...
            bool newButtonStatus = Buttons[buttonID] == 1;
            bool oldButtonStatus = buttonStatus;
            bool olderButtonStatus = buttonFilter;
            // Log button status for next cycle
            buttonFilter = oldButtonStatus;
            buttonStatus = newButtonStatus;

            if (oldButtonStatus == false && olderButtonStatus == false && newButtonStatus == true) // were the previous two statusses unpressed?
            {
                if (grabbing)
                    release();
                else
    ...
    #region GrabbingScripts
    void grab()
    {
        for (int i = 0; i < 3; i++) // Do it 3 times, not exactly elegant but fixed it so it connects nicely every time (even when moving)
        {
            getRefInfo();
            updateDevice();
            updateManipulator();
        ...
        if ((controlPoint - hapticManipulator.transform.position).magnitude > 0.3f)
        {
            Debug.Log("Still too far away, connection went wrong.");
            return;
        }
        ...
        /*if (controlledObject != null) // This is broken
            Debug.Log("Not grabbing anything, did you indicate a controlled Object?");*/


        // if everything went alright: Create a joint to grab the object
        Destroy(jointPause);
        grabbing = controlledObject;

        Rigidbody body = grabbing.GetComponent<Rigidbody>();

        joint = (ConfigurableJoint)hapticManipulator.AddComponent(typeof(ConfigurableJoint));
        // lock all DOFs
        ...
        //joint.anchor = -relativeControlPoint;s
        //joint.anchor
        joint.connectedBody = body;
        joint.connectedMassScale = 8; // make the control less/more heavy

        // turn on the physics
        gameObject.GetComponent<HapticPlugin>().PhysicsManipulationEnabled = true;
    }

    public void release() // Release the currently grabbed item
    {
        ...
        gameObject.GetComponent<HapticPlugin>().PhysicsManipulationEnabled = false; // turn off the force feedback

        jointPause = (FixedJoint)controlledObject.AddComponent(typeof(FixedJoint));

        // turn on the physics
        gameObject.GetComponent<HapticPlugin>().PhysicsManipulationEnabled = false;
    }

    private void getRefInfo() // Find the correction required to make position haptic manipulator = position controlled object
    {
        updateDevice();

        Vector3 manipPosCorrOld = manipPosCorr;
        manipPosCorr = controlPoint - hapticManipulator.transform.position + manipPosCorrOld;

        omniPosCorr = stylusPositionRaw - gameObject.transform.InverseTransformPoint(hapticManipulator.transform.position);
    }
    #endregion
    ...
    private void updateDevice()
    {
        ...
        stylusPositionWorld += manipPosCorr;
        stylusRotationWorld = QuaternionFromMatrix(stylusMatrixWorld); // = stylusrotationRaw

        stylusVelocityWorld = gameObject.transform.InverseTransformVector(stylusVelocityRaw);

        OmniSpring.stylusPositionRaw = stylusPositionRaw;
        OmniSpring.stylusVelocityRaw = stylusVelocityRaw;
        OmniSpring.stylusRotationWorld = stylusRotationWorld;
    ...
    private void updateManipulator()
    {
        ...
        Vector3 springPos = gameObject.transform.InverseTransformPoint(this.hapticManipulator.transform.position) + omniPosCorr;
        ...
        loggedSetSpringAnchorPosition(configName, springPosOut, springVelOut); // logging versions of these functions
        loggedSetSpringStiffness(configName, this.PhysicsForceStrength, 0);
    ...
    public void safeUpdateManipulator()
    {
        ...
        {
            if (!float.IsNaN(stylusPositionWorld.x)) // For some reason sometimes the x position is NaN
                body.position = stylusPositionWorld;
            body.rotation = stylusRotationWorld;
        ...
        double[] zero = { 0.0, 0.0, 0.0 };
        loggedSetSpringStiffness(configName, 0.0, 0.0);
        setForce(configName, zero, zero);
    }

    #region LoggingFunctions
    void loggedSetSpringStiffness(string configName, double stiffness, double damping)
    {
        setSpringStiffness(configName, stiffness, damping);
        OmniSpring.configName = configName;
        OmniSpring.stiffness = stiffness;
        OmniSpring.damping = damping;
    }

    void loggedSetSpringAnchorPosition(string configName, double[] anchorPosition3, double[] anchorVelocity3)
    {
        setSpringAnchorPosition(configName, anchorPosition3, anchorVelocity3);
        OmniSpring.configName = configName;
        OmniSpring.anchorPosition3 = anchorPosition3;
        OmniSpring.anchorVelocity3 = anchorVelocity3;
    }
