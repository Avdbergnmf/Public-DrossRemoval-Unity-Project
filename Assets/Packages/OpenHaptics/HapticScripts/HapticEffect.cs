	...
	void OnDestroy()
	{
		...
		for (int ii = 0; ii < devices.Length; ii++)
		{
			...
			if (device == null)
				continue;
			int ID = FXID [ii];
			HapticPlugin.effects_stopEffect(device.configName, ID); // gives naggy error if effect wasn't used but better to be sure its deleted. Could find a way to prevent this but meh...
		}
	}
	void OnDisable()
	{
